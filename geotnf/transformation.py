...
class GeometricTnf(object):
    ...
    def __init__(self, geometric_model='affine', out_h=240, out_w=240, use_cuda=True):
        ...
        if geometric_model=='affine':
            ...
            self.gridGen = TpsGridGen(out_h, out_w, grid_size=3, use_cuda=use_cuda)
...
class TpsGridGen(Module):
    ...
    def compute_L_inverse(self,X,Y):
        ...
        K = torch.mul(P_dist_squared,torch.log(P_dist_squared)) + 0.0*torch.eye(N)
        # construct matrix L
    ...
    def apply_transformation(self,theta,points):
        #### the following code will be commented for experiments
        if theta.dim()==2:
