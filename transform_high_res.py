...
def transform_high_res(transforms, path, use_cuda=True):
    ...
    source_image_var = process_image(source_image, use_cuda, half_out_size=1024, high_res=True)

    # Affine transformations
    warped_image_aff = affTnf_high_res(source_image_var, theta_aff_1.view(-1,2,3))
    warped_image_aff = affTnf_high_res(warped_image_aff, theta_aff_2.view(-1,2,3))
   
    # TPS transformation
    warped_image_aff_tps = tpsTnf_high_res(warped_image_aff,theta_tps)
...
def main():
    for sid in samples:
        ...
        for annot in all_paths:
            warped_imgs[annot] = np.zeros((count, w, h, 3))
