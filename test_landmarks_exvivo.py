...
# Paths
parser.add_argument('-t', '--test-csv-name',       type=str, default='landmarks_mri.csv',  help='test data csv file name')
...
dataset_mri = LandmarkDataset(geometric_model    = geometric_model,
                            csv_file             = './training_data/tps/landmarks_mri.csv',
                            training_image_path  = args.training_image_path,
                            batch_size           = args.batch_size,
                            transform            = NormalizeImageDict(['source_image','target_image']),
                            mri                  = True)

dataset_histo = LandmarkDataset(geometric_model  = geometric_model,
                            csv_file             = './training_data/tps/landmarks.csv',
                            training_image_path  = args.training_image_path,
                            batch_size           = args.batch_size,
                            transform            = NormalizeImageDict(['source_image','target_image']),
                            mri                  = False)

dataloader_mri   = DataLoader(dataset_mri,   batch_size=args.batch_size, shuffle=False, num_workers=4)
dataloader_histo = DataLoader(dataset_histo, batch_size=args.batch_size, shuffle=False, num_workers=4)
...
for batch_idx, batch in enumerate(dataloader_mri):
    ...
    # Save images
    save_images = {
        "exvivo_":           warped_histo,
        "exvivo_source_":    tnf_batch['source_image'], 
    }
    ...
    mri_dice_loss += dice_loss
    
mri_dice_loss     /= len(dataloader_mri)
print('Average DICE loss: {:.6f}'.format(mri_dice_loss))


for batch_idx, batch in enumerate(dataloader_histo):
    tnf_batch       = landmark_tnf(batch)
    ...
    warped_histo            = aff_tnf(warped_histo, theta_aff_1)
    warped_mask             = aff_tnf(warped_mask,  theta_aff_1)
    warped_img_landmarks    = aff_tnf(warped_img_landmarks,  theta_aff_1)
    ...
    dice_loss        = calc_dice(tnf_batch['source_mask'][:,0,:,:], tnf_batch['target_mask'][:,0,:,:])
    histo_dice_loss += dice_loss
    
total_landmark_loss /= len(dataloader_histo)
histo_dice_loss     /= len(dataloader_histo)
print('\nAverage landmark SSD loss: {:.6f}'.format(total_landmark_loss))
print('Average DICE loss: {:.6f}'.format(histo_dice_loss))
